...
CABECALHO = " "*7 + "n" + " "*5 + "sort  mergesortR  quicksortX  quicksort"
MIN = 2**10
MAX = 2**26
...
def main():
    ...
    while i <= MAX:
        ...
        #----------------------------------------
        # ordenação por quicksortX recursivo
        v = v_orig.copy()
        t_quicksortX = execute(quicksortX, v) 
        for j in range(i):
            if v[j] != j:
                print("SOCORRO! mergesort(v) não ordenou v!")
                return
        ...
        # mostre resultados
        print(f"{i:9} {t_sort:7.2f}{t_mergesortR:10.2f}{t_quicksortX:11.2f}{t_quicksortI:11.2f}")
